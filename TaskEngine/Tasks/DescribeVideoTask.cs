...
namespace TaskEngine.Tasks
{
    ...
    class DescribeVideoTask : RabbitMQTask<string>
    {
        ...
        public DescribeVideoTask(RabbitMQConnection rabbitMQ, DescribeImageTask task, RpcClient rpcClient, ILogger<DescribeVideoTask> logger)
            : base(rabbitMQ, TaskType.DescribeVideo, logger)
        {
            _describeImageTask = task;
        }
        ...
        protected async override Task OnConsume(string videoId, TaskParameters taskParameters, ClientActiveTasks cleanup)
        {
            ...
            GetLogger().LogInformation($"SceneDescriptionTask({videoId}): Consuming Task");
            ...
            using (var _context = CTDbContext.CreateDbContext())
            {
                ...
                foreach (JObject scene in scenes) {
                    string? captionId = scene["captionId"].ToString();
                    var caption = captions.Where(c=>c.Id==captionId).FirstOrDefault();
                    if( caption == null) {       
                        var c = new Caption
                            {
                                Index = CaptionIndex++,
                ...
                if(newCaptions.Any()) {
                    ...
                    foreach(int i in scenesWithNewCaption) {                    
                        scenes[i]["captionId"] = newCaptions[i].Id;
                    }
                ...
                foreach (int i in describeScenes) {
                    ...
                    string imageFilePath = scene[""].ToString();
                    string captionId = scene["captionId"].ToString();

                    taskEnqueueCount ++;
                    // Todo Create Parameter imageFilePath, captionId,transcription.Id
                    var taskParams = new TaskParameters();
                    
                    GetLogger().LogInformation($"Describe Video {videoId}: {imageFilePath} {captionId} {transcription.Id}");            
                    _describeImageTask.Publish(imageFilePath,taskParams);
